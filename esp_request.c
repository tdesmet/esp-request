...
#define REQ_CHECK(check, log, ret) \
    if (check)                     \
    {                              \
        ESP_LOGE(REQ_TAG, log);    \
        ret;                       \
    }
        ...
        if (process_header == 0)
        {
            ...
            if (chunked)
            {
                if ((req->buffer->bytes_write - header_off) != 0)
                {
                    ...
                    {
                        if (req->download_callback(req, (void *)(req->buffer->data + req->buffer->bytes_read), chunk_length) < 0)
                            break;
            ...
            else
            {
                ...
                {
                    if (req->download_callback(req, (void *)(req->buffer->data + header_off), req->buffer->bytes_write - header_off) < 0)
                        break;
